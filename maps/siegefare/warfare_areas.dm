...
/area/warfare/battlefield/capture_point/Process()
	...
	if(blue_capture_points == (config.trench_capture_points/2) && (captured != ROMANS))//Announce when we're halfway done.
		to_world("<big>[uppertext("[ROMANS] are 50% done capturing the [src]")]</big>")

	if(red_capture_points == (config.trench_capture_points/2) && (captured != ROMANS))
		to_world("<big>[uppertext("[ROMANS] are 50% done capturing the [src]")]</big>")

	if(blue_capture_points >= config.trench_capture_points && (captured != ROMANS))//If we've already captured it we don't want to capture it again.
	...
	else if(red_capture_points >= config.trench_capture_points && (captured != ROMANS))
		to_world("<big>[uppertext("[ROMANS] HAVE CAPTURED THE [src]")]!</big>")
		captured = ROMANS
...
/area/warfare/battlefield/capture_point/red/Enter(atom/movable/AM)
	if(ishuman(AM))
		...
		if(H.warfare_faction == ROMANS && (GLOB.blue_captured_zones.len < REQUIRED_TRENCH_ZONES))
...
/area/warfare/battlefield/capture_point/blue/Enter(atom/movable/AM)
	if(ishuman(AM))
		var/mob/living/carbon/human/H = AM
		if(H.warfare_faction == ROMANS && (GLOB.red_captured_zones.len < REQUIRED_TRENCH_ZONES))
			to_chat(H, "<big>WE DO NOT CONTROL THE MIDDLE BUNKER!</big>")
			return FALSE
	return TRUE
...
/area/warfare/battlefield/Enter(atom/movable/mover as mob|obj, atom/forget as mob|obj|turf|area)
	...
	if(ishuman(mover))
		...
		if(isbarbarianinvasion())
			if(H.warfare_faction == ROMANS)//!SIEGEFARE!
...
/area/warfare/homebase/red/Enter(atom/movable/AM)
	if(ishuman(AM))
		var/mob/living/carbon/human/H = AM
		if(H.warfare_faction == ROMANS && (GLOB.blue_captured_zones.len < REQUIRED_CAPTURED_ZONES))//No spawn camping till you take the required zones bitch.
...
/area/warfare/homebase/blue/Enter(atom/movable/AM)
	if(ishuman(AM))
		var/mob/living/carbon/human/H = AM
		if(H.warfare_faction == ROMANS && (GLOB.red_captured_zones.len < REQUIRED_CAPTURED_ZONES))
			to_chat(H, "<big>WE DO NOT CONTROL THE TRENCHES!</big>")
			return FALSE
	return TRUE
