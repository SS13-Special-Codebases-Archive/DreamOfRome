/mob/living/carbon/human/proc/help_me()
	...
	var/is_blue = SSjobs.GetJobByTitle(job).is_barbarians_team
	var/class = "red_team"
	var/datum/team/T = SSwarfare.romans
	if(is_blue)
		class = "blue_team"
		T = SSwarfare.barbarians
...
/mob/living/carbon/human/proc/check_reinforcements()
	...
	if(T.checkCooldown("Check Reinforcements"))
		...
		return
	if(is_blue)
		to_chat(src, "<span class='bnotice'><font size=4>Reinforcements Left: [SSwarfare.barbarians.left]</font></span>")
	else
		to_chat(src, "<span class='bnotice'><font size=4>Reinforcements Left: [SSwarfare.romans.left]</font></span>")
