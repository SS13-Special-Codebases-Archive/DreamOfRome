...
/client/Del()
	...
	GLOB.clients -= src
	if(warfare_faction == ROMANS)//remove them from the list if they disconnect, so we don't fuck the list up.
		SSwarfare.romans.team_clients -= src
	else
		SSwarfare.barbarians.team_clients -= src
	return ..()
