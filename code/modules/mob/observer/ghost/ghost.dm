...
/mob/observer/ghost/MayRespawn(var/feedback = 0, var/respawn_time = 0)
	...
	*/
	...
	if(isbarbarianinvasion())//If it's invasion then respawn is cut down to the config time, which is usually 2 minutes.
		respawn_time = config.warfare_respawn_time
		if(client.warfare_faction == ROMANS)
			if(GLOB.roman_captured_zones.len < 1)
				to_chat(src, "We control no camps, we cannot respawn.")
				return FALSE
		else if(client.warfare_faction == BARBARIANS)
			if(GLOB.barbarian_captured_zones.len < 1)
				to_chat(src, "We control no camps, we cannot respawn.")
				return FALSE
