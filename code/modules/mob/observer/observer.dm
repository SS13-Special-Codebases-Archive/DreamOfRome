...
/mob/observer/Stat()
	...
	if(statpanel("Status"))
		if(isbarbarianinvasion())
			if(client?.holder)
				stat("[BARBARIANS] reinforcements:", SSwarfare.barbarians.left)
				//stat("[ROMANS] capture points:", SSwarfare.barbarians.points)
				stat("[ROMANS] reinforcements:", SSwarfare.romans.left)
				//stat("[ROMANS] capture points:", SSwarfare.romans.points)

			for(var/area/A in GLOB.roman_captured_zones)
				stat("Roman Captured Zones:", A)
			for(var/area/A in GLOB.barbarian_captured_zones)
				stat("Barbarian Captured Zones:", A)
