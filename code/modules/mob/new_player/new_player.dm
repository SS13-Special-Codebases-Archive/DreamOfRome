...
/mob/new_player/proc/IsJobAvailable(var/datum/job/job)
	...
	if(job.is_romans_team)//Can't join the team if they have more people on their side.
		if(client?.warfare_faction != ROMANS)
			return 0
	if(job.is_barbarians_team)
		if(client?.warfare_faction != BARBARIANS)
			return 0
...
/mob/new_player/proc/LateChoices()
	...
	var/list/jobcats = list(  // This is designed for warfare, in a warfare-less environment, this will need to be changed
		"The Roman Empire"=list("color"="#b27676", "jobs"=list()),
		"Enemies of the Empire"=list("color"="#76abb2", "jobs"=list()),
		)
	...
	for(var/datum/job/job in SSjobs.occupations)
		...
		if(job.is_barbarians_team)
			jobcats["Enemies of the Empire"]["jobs"] += job
		else if(job.is_romans_team)
			jobcats["The Roman Empire"]["jobs"] += job
...
/mob/new_player/proc/show_new_information()
	var/dat = {"<h2>Dream of Rome!</h2>
Hello! It appears you are new here! Thanks for joining Dream of Rome. Here is some important information to get you started!
<h3>Discord Link</h3>
<font size='4'><a href=\"https://discord.gg/NF62KPjMSx\">Discord</a></font>
<h3>Server Summary</h3>
Dream of Rome is a team deathmatch game set in the chaotic era of Late Antiquity. You can be part of the decaying Roman army or the assault barbarian hordes (possible Persian additions in the future), and your objective is to either deplete all the other sides reinforcements, or take over their camps and overrun their army. The barbarians show no mercy, and you are re-enacting the ancient form of human wave tactics.
<h3>Some Notes</h3>
Chat bar commands in game do not work. You will have to use the panels on the side if you wish to use any commands. Typing in the command bar will use OOC if you are in the lobby, and it will use say automatically if you're in game.
