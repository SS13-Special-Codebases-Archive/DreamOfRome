	...
	spawn(40)
		if(client)
			...
			if(!client.warfare_faction)
				...
				if(SSwarfare.barbarians.team_clients.len*2 < SSwarfare.romans.team_clients.len)
					client.warfare_faction = ROMANS
					SSwarfare.romans.team_clients += src.client
				else
					client.warfare_faction = BARBARIANS
					SSwarfare.barbarians.team_clients += src.client
